    ...
    return () => <div class="editor-container">
      ...
      >
        <el-form-item label="用户名" prop="username">
          <el-input v-model={editData.value.username} placeholder="请输入用户名" />
        </el-form-item>
        <el-form-item label="手机" prop="mobile">
          <el-input
            v-model={editData.value.mobile}
            placeholder="请输入手机"
            maxlength="11"
          />
        </el-form-item>
        <el-form-item label="邮箱" prop="email">
          <el-input v-model={editData.value.email} placeholder="请输入邮箱" />
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-switch v-model={editData.value.status} />
        </el-form-item>
        <el-form-item label="角色分配" prop="roleIds">
          <el-select multiple v-model={editData.value.roleIds} placeholder="请选择角色">
            {
              editData.value.roles && editData.value.roles.map((item: any) => <el-option
                label={item.name}
                value={item.id}
              ></el-option>)
            }
        ...
        <el-form-item label="说明" prop="description">
          <el-input
            type="textarea"
            rows={3}
            v-model={editData.value.description}
            placeholder="请输入说明"
          />
        </el-form-item>
        <el-form-item>
